...
from cerberus_re_skill.core.platform_helpers import (
    get_config_home,
    get_platform,
    get_skill_root,
    get_workspace_root,
...
class Config:
    ...
    def reload(self) -> None:
        ...
        self.triage_manifest: Path = Path(
            _env(
                "GHIDRA_RE_TRIAGE_MANIFEST",
                str(self.skill_root / "references" / "triage-patterns.json"),
            )
        )
        ...
        self.notes_repo: str = _env("GHIDRA_NOTES_REPO", "")
        self.notes_issue_title: str = _env("GHIDRA_NOTES_ISSUE_TITLE", "Global Use-Case Driven Notes")
        self.notes_issue_number: str = _env("GHIDRA_NOTES_ISSUE_NUMBER", "")
