...
from cerberus_re_skill.core.subprocess_utils import find_tool, run, run_output
from cerberus_re_skill.core.utils import flag_enabled, load_json, utc_now, write_json


def _python() -> str:
    from cerberus_re_skill.core.subprocess_utils import find_python
    return find_python()
...
def _notes_repo_configured() -> bool:
    return bool(cfg.notes_repo.strip())
    ...
    if not cfg.notes_config_file.exists():
        ...
        payload = {
            ...
            "issue_url": (
                ...
                if cfg.notes_repo and issue_number
                else ""
            ),
...
def add(
    ...
    if _notes_enabled() and _auto_sync_enabled() and _notes_repo_configured() and _gh_authenticated():
        try:
            sync_result = sync()
            added["synced"] = True
...
def sync() -> dict:
    ...
    init_files()
    if not _notes_repo_configured():
        raise RuntimeError("GHIDRA_NOTES_REPO is not configured")
...
def open_shared() -> None:
    ...
    if not url and cfg.notes_repo and cfg.notes_issue_number:
