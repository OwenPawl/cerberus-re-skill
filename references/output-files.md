...
- `probe-plan.json`, `probe-lifecycle.json`, `gadget-output.txt`, `gadget-report.json`: content-addressed Frida Gadget launch plan, append-only lifecycle observations, raw autonomous-script output, and structured result. Gadget hits prove injection into the owned launched copy; they do not prove task-port attachment is available. Rejection and crash evidence is retained but remains non-passing.
